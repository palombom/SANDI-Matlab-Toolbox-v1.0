...
if(strcmp(protocol.pulseseq, 'PGSE') || strcmp(protocol.pulseseq,'STEAM'))
    ...
    % get the relevalt pusle sequence parameters from protocol
        ...
        bval1 = GAMMA^2*(GD(:,1).^2.*smalldel.^2.*tdd + cG(:,1).^2.*sdelc.^2.*tcc + ...
    rG(:,1).^2.*sdelr.^2.*trr + 2*GD(:,1).*cG(:,1).*smalldel.*sdelc.*tdc + ...
    2*GD(:,1).*rG(:,1).*smalldel.*sdelr.*tdr + 2*cG(:,1).*rG(:,1).*sdelc.*sdelr.*tcr);
    
         bval2 = GAMMA^2*(GD(:,2).^2.*smalldel.^2.*tdd + cG(:,2).^2.*sdelc.^2.*tcc + ...
    rG(:,2).^2.*sdelr.^2.*trr + 2*GD(:,2).*cG(:,2).*smalldel.*sdelc.*tdc + ...
    2*GD(:,2).*rG(:,2).*smalldel.*sdelr.*tdr + 2*cG(:,2).*rG(:,2).*sdelc.*sdelr.*tcr);

        bval3 = GAMMA^2*(GD(:,3).^2.*smalldel.^2.*tdd + cG(:,3).^2.*sdelc.^2.*tcc + ...
    rG(:,3).^2.*sdelr.^2.*trr + 2*GD(:,3).*cG(:,3).*smalldel.*sdelc.*tdc + ...
    2*GD(:,3).*rG(:,3).*smalldel.*sdelr.*tdr + 2*cG(:,3).*rG(:,3).*sdelc.*sdelr.*tcr);
